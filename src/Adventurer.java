	...
	public boolean canSee(int x, int y)
	{
		if(Math.pow(Math.abs(x-xPos),2)+Math.pow(Math.abs(y-yPos),2)<=Math.pow(visionRange,2))
		{
			for(int i=Math.min(x,xPos)+1;i<Math.max(x,xPos);i++)
				if(map.getTile(i,yPos,2).getType()==2 || (map.getTile(i,yPos,2).getType()==4 && !((DoorTile)(map.getTile(i,yPos,2))).isOpen()))
					return false;
			for(int i=Math.min(y,yPos)+1;i<Math.max(y,yPos);i++)
				if(map.getTile(xPos,i,2).getType()==2 || (map.getTile(xPos,i,2).getType()==4 && !((DoorTile)(map.getTile(xPos,i,2))).isOpen()))
					return false;
			return true;
		}
		return false;
